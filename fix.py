...
# Build step se pehle safe official generator inject
generator = """
      - name: Generate Safe Clean Main Code
        run: |
          mkdir -p lib
          cat << 'END_DART' > lib/main.dart
              ...
              showDialog(
                ...
                builder: (ctx) => AlertDialog(
                  ...
                  actions: [
                    ...
                    ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFFD32F2F)), onPressed: () {
                    ...
                    Expanded(
                      child: StreamBuilder<QuerySnapshot>(
                        ...
                        builder: (context, snap) {
                          ...
                          final docs = snap.data!.docs.where((d) => cat == 'All' || (d.data() as Map<String, dynamic>)['category'] == cat).toList();
...
idx = content.find("- name: Build APK")
if idx != -1:
    new_content = content[:idx] + generator + "\n      " + content[idx:]
    with open(".github/workflows/build_apk.yml", "w") as f:
        f.write(new_content)
    print("WORKFLOW_UPDATED_SUCCESSFULLY")
