    ...
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          if (orders.isEmpty) {
            return const Center(child: Text('Abhi koi active delivery nahi hai!'));
          }
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text('Address: $address', style: const TextStyle(color: Colors.black87)),
                      Text('Phone: $phone', style: const TextStyle(color: Colors.blueGrey, fontWeight: FontWeight.bold)),
                      Text('Payment: ${d["paymentMethod"] ?? "COD"} | ₹$total', style: const TextStyle(fontWeight: FontWeight.bold, color: Color(0xFFFC8019))),
                      const Divider(),
                      Text('Items: $itemsText', style: const TextStyle(fontSize: 13, color: Colors.black54)),
                      const SizedBox(height: 12),
                      Row(
                        children: [
                          if (status != 'Out for Delivery')
                            Expanded(
                              child: ElevatedButton(
                                style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFFFC8019), foregroundColor: Colors.white),
                                onPressed: () {
                                  FirebaseFirestore.instance.collection('orders').doc(doc.id).update({'status': 'Out for Delivery'});
                                },
                                child: const Text('Pick Delivery'),
                              ),
                            ),
                          if (status != 'Out for Delivery') const SizedBox(width: 8),
                          Expanded(
                            child: ElevatedButton(
                              style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF60B244), foregroundColor: Colors.white),
                              onPressed: () {
                                FirebaseFirestore.instance.collection('orders').doc(doc.id).update({'status': 'Delivered'});
                              },
